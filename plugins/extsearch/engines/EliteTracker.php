...
class EliteTrackerEngine extends commonEngine
{
	...
	public $categories = array( 'all'=>'', "Animations/Animes" => "27", "Application" => "74", "Documentaires" => "38", "Ebooks" => "34",
				"Films" => "7", "HD" => "48", "Jeux Vidéo" => "15", "Musiques" => "23", "Séries" => "30", "Spectacles/Émissions" => "30", "Sport" => "35", "XXX" => "37" );
	...
	public function action($what,$cat,&$ret,$limit,$useGlobalCats)
	{
		...
		for($pg = 0; $pg<10; $pg++)
		{
			...
			if( ($cli==false) || (strpos($cli->results, "<b>Aucun Resultat</b>")!==false) ||
				...
				break;
			$res = preg_match_all('/<td .* class="unsortable2">.*<a href=".*\.ts"><img src=".*\/images\/.*" border="0" alt="(?P<cat>.*)".*\/><\/a>.*<\/td>'.
                                '.*<td .*>.*<a href="https:\/\/.*\/(?P<name>.*)-s-(?P<id>\d+)\.ts">.*<\/a>'.
                                '.*<img src=.*\/>.*<\/span>(?P<date>.*)<\/div>.*<\/td>.*<td .*>.*<\/td>.*<td .*>.*<\/td>.*<td .*>(?P<size>.*)<\/td>'.
                                '.*<td .*>.*<span .*>.*<a href=".*>(?P<seeds>.*)<\/a>.*<\/span>.*<\/td>.*<td .*>.*<span .*>.*<a href=".*>(?P<leech>.*)<\/a>/siU', $cli->results, $matches);
			...
			if($res)
			{
				for($i = 0; $i < $res; $i++)
				{
					...
					$link = $url.'/'.$matches["name"][$i].'-d-'.$matches["id"][$i].'.ts';
					if(!array_key_exists($link,$ret))
					{
						...
						$item["time"] = strtotime(self::removeTags(str_replace("<br />"," ",$matches["date"][$i])))+1;
