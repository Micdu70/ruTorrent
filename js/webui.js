...
//				if((oldTorrent.seeds!=torrent.seeds) || (oldTorrent.peers!=torrent.peers))
	...
	updateViewRows: function(table)
	{
		...
		$('#viewrows_size').text(theConverter.unit(viewSize, 2));
	},
...
//
	...
	updateLabel: function(label, count, size, showSize) {
		var li = $(label);
		var lblSize = theConverter.unit(size, 2);
...
//
	...
	downRateMenu: function(e)
	{
	        if(e.which==3)
	        {
	                ...
	                if(theWebUI.total.rateDL==0)
	                	theContextMenu.add([CMENU_SEL,theUILang.unlimited,"theWebUI.setDLRate(0)"]);
			else
		                theContextMenu.add([theUILang.unlimited,"theWebUI.setDLRate(0)"]);
			theContextMenu.add([CMENU_SEP]);
	...
	upRateMenu: function(e)
	{
	        if(e.which==3)
	        {
	                ...
	                if(theWebUI.total.rateUL==0)
	                	theContextMenu.add([CMENU_SEL,theUILang.unlimited,"theWebUI.setULRate(0)"]);
			else
		                theContextMenu.add([theUILang.unlimited,"theWebUI.setULRate(0)"]);
			theContextMenu.add([CMENU_SEP]);
	                for(var i=0; i<speeds.length; i++)
	                {
	                	var spd = iv(speeds[i])*1024;
